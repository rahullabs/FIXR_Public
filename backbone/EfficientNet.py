...
class EfficientNet(MammothBackbone):
    ...
    def __init__(self, blocks_args=None, global_params=None, hookme=False):
        ...
        self._avg_pooling = nn.AdaptiveAvgPool2d(1)
        in_channels_to_fc = round_filters(512, self._global_params)
        
        if self._global_params.include_top:
            self.fc = nn.Linear(out_channels, in_channels_to_fc)
            self._dropout = nn.Dropout(self._global_params.dropout_rate)
            self.classifier = nn.Linear(in_channels_to_fc, self._global_params.num_classes)
...
def mammoth_efficientnet(nclasses: int, model_name: str, pretrained=True):
    """
    ...
    if not pretrained:
        ...
        efp = EfficientNet.from_pretrained(model_name=model_name, weights_path="./backbone/efficientnet_weight/adv-efficientnet-b0-b64d5a18.pth", num_classes=nclasses)
        
        # param_list = list(efp.parameters())
        # for idx in range(len(param_list)-2):
        #     param_list[idx].requires_grad = False
